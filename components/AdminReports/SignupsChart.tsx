// components/AdminReports/SignupsChart.tsx
'use client'

import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip } from 'recharts'
import type { DayCount } from '@/lib/admin-reports'

function fmtDate(iso: string) {
  return new Date(iso + 'T00:00:00Z').toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })
}

function CustomTooltip({ active, payload }: { active?: boolean; payload?: { payload: DayCount }[] }) {
  if (!active || !payload || payload.length === 0) return null
  const row = payload[0].payload
  return (
    <div className="bg-bg3 border border-border rounded-lg p-3 shadow-lg">
      <div className="text-xs text-white font-bold mb-1">{fmtDate(row.date)}</div>
...
export default function SignupsChart({ data }: { data: DayCount[] }) {
  return (
    <div className="card h-[280px] flex flex-col">
      <div className="stat-label mb-3">Signups — Last {data.length} Days</div>
