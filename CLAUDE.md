...
- **No live Supabase credentials in the sandbox, by default.** `next build`/`next dev` need at least syntactically-valid env vars (`proxy.ts` calls `createServerClient()` on every request) — use placeholder values for local verification, never assume a real backend is reachable. (One-off exception, 2026-09-22: real credentials were pasted into chat and written to `.env.local` — gitignored, never committed — to verify PR7 against production data; see that verification's approach below. The sandbox container is ephemeral, so a fresh session starts without them again — don't assume they're still there.)
- **Verifying against real data, when credentials are available**: prefer exercising the actual `lib/` functions (not a reimplementation) via `npx tsx --env-file=.env.local <script>.ts` — `lib/supabase/admin.ts`'s `createAdminClient()` reads `process.env` lazily inside the function, so this works without needing dotenv as a dependency. For read paths, just call the same queries the page/route does. For a write path, capture the original value, write, confirm, then write the exact original back and diff — never leave a live record only spot-checked, and never test a hard delete against real data (no clean revert) unless the user explicitly OKs losing that record. Always ask before the first write against real data, even when reads were already pre-approved. Delete any temp verification scripts afterward — they tend to have real record IDs hardcoded in them.
